...
Data=pd.read_csv(r'./kaliu python/Data1.csv')#read table
...
From the analysis before, we can get the following results. From the predictors given, the `age`, `systolic`, `diastolic`, `weight`, `height`, `bmi`, `triglycerides`, $triglycerides^2$, $age^2$ are the significant predictors taht effect the level of `ldl`; the `age`, `systolic`, `diastolic`,`weight` and `triglycerides` predictors have the positive effect, and `height` and `bmi` have the negative effect. For `triglycerides` and `age`, with the predictors increase, the level of $\sqrt{ldl}$ with increase slower and slower, because the coefficient of $triglycerides^2$ and $age^2$ are negative.
